...
algoarray <- c(new('NN-R'), new("RF"), new('SVM-R')) ### algorithms used for fitting and prediction
...
Result <- foreach(iter = 1:ns, .combine = "rbind", .packages = c('MASS', 'glmnet', "caret", "kernlab", "randomForest", "ks", "nnet"), .errorhandling = "remove")%dopar% {
  ...
  Y <- 10*X[,1]+7*X[,2]^2+7*exp(X[,3]+2*X[,4]^2)+rnorm(n+N, sd = 1)
  data <- data.frame(x = X, y = Y)
  Value <- list(type = ">=A&<=B", v = c(quantile(data$y, 0.1), quantile(data$y, 0.9)))
  
  datawork <- DataSplit(data, n+N, N, n_cal)
  ...
  for (algo in algoarray) {
    ...
    #---PASS---#
    TN <- Tcompute(W_cal, W_test, Null_cal, algo, h1 = 0, h2 = 0)
    ...
    info <- rbind(info, list(
      FDP = Record_PASS$FDP, SIM = Record_PASS$SIM, Method = "PASS", Algorithm = algo@name, datasetting = "REG"
    ))
    
    
    #---PASSC---#
    TN_cv <- KfoldTcompute(X_rest, Y_rest, X_test, Null_rest, W_test_whole, K, lambda, algo, h1 = 0, h2 = 0)
    ...
    info <- rbind(info, list(
      FDP = Record_PASSC$FDP, SIM = Record_PASSC$SIM, Method = "PASSC", Algorithm = algo@name, datasetting = "REG"
    ))
    ...
    info <- rbind(info, list(
      FDP = Record_CP$FDP, SIM = Record_CP$SIM, Method = "CP", Algorithm = algo@name, datasetting = "REG"
    ))
    
    
    #---SSD---#
    Screened_pred <- setdiff(1:N, NullIndex(W_test_whole, Value)) 
    ...
    info <- rbind(info, list(
      FDP = Record_SSD$FDP, SIM = Record_SSD$SIM, Method = "SSD", Algorithm = algo@name, datasetting = "REG"
    ))
    ...
    info <- rbind(info, list(
      FDP = Record_SS$FDP, SIM = Record_SS$SIM, Method = "SS", Algorithm = algo@name, datasetting = "REG"
    ))
    ...
    info <- rbind(info, list(
      FDP = Record_loc$FDP, SIM = Record_loc$SIM, Method = "LocF", Algorithm = algo@name, datasetting = "REG"
    ))
  }
...
pp <- Result%>%
  ...
  dplyr::summarize(FDR = mean(FDP), SIMmean = mean(SIM), sdFDR = sd(FDP)/sqrt(length(Result[, 1])), SIMsd = sd(SIM)/sqrt(length(Result[, 1])))
pp


### plotting
Result$datasetting <- "Setting A"
Result$Algorithm[Result$Algorithm=="NN-R"] <- "NN"
Result$Algorithm[Result$Algorithm=="SVM-R"] <- "SVM"
...
P2 <- ggplot(data = Result, aes(x = Method, y = SIM, fill = Method)) +
  geom_boxplot(alpha = 0.7) +
  scale_y_continuous(name = "ES", limits = c(0.03, 0.102)) +
  scale_x_discrete(name = "Method") +
  theme_bw() +                
  stat_summary(mapping = aes(group = Method),                    
               fun = "mean",                                   
...
PP <- ggarrange(P1, P2)
PPR <- annotate_figure(PP, top = text_grob("Scenario A", color = "black", face = "bold", size = 14))
PPR
